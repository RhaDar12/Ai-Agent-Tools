...
def fetch_market_bundle(symbol):
    """
    Mengambil bundle data H1 dan M15 untuk strategi.
    """
    tool_name = "fetch_market_bundle"

    try:
        h1 = fetch_ohlc(symbol, interval="1h", period="7d")
        m15 = fetch_ohlc(symbol, interval="15m", period="5d")

        if not h1.get("success"):
            raise RuntimeError(h1.get("error", "Gagal mengambil H1"))

        if not m15.get("success"):
            raise RuntimeError(m15.get("error", "Gagal mengambil M15"))

        return success_response(
            tool=tool_name,
            message="Bundle market berhasil diambil",
            extra={
                "symbol": symbol.upper().strip(),
