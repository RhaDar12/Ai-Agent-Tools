...
def _is_allowed_session():
    ...
    london_open = 8 <= hour < 11
    london_ny_overlap = 12 <= hour < 16
    asian_forbidden = hour >= 22 or hour < 6

    return {
        "allowed": (london_open or london_ny_overlap) and not asian_forbidden,
        "hour_gmt": hour,
...
def _find_prior_24h_high_low(h1):
    if len(h1) < 25:
        raise ValueError("Data H1 kurang dari 25 candle untuk menghitung prior 24H high/low.")
...
def _detect_sweep(symbol, h1):
    """
    Cek sweep dalam 4 candle H1 terakhir.
    Bullish:
...
def analyze_liquidity_sweep(symbol, news_status="Manual check required", spread_pips=None):
    ...
    try:
        ...
        bundle = fetch_market_bundle(symbol)
